...
class EmailService {
  ...
  private async sendEmailDirect(data: EmailJob): Promise<void> {
    try {
      ...
      // Note: HTML content is trusted (generated by our templates)
      // Plain text version is optional and only used as fallback
      await this.transporter.sendMail({
        from: `${fromName} <${fromEmail}>`,
        to: data.to,
        subject: data.subject,
        html: data.html,
        text: data.text, // Use provided text or let nodemailer handle it
      });
